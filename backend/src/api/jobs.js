...
router.get('/:uuid/results', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT id, email, first_name, last_name, verdict, reason,
              host(ip_used)::text AS ip_used, duration_ms, status, verified_at
       FROM verifications
       WHERE bulk_job_id = $1
       ORDER BY id
       LIMIT $2 OFFSET $3`,
...
router.get('/:uuid/results.csv', async (req, res) => {
  try {
    ...
    const csv = stringify(rows, {
      header: true,
      columns: ['first_name', 'last_name', 'email', 'verdict', 'reason', 'ip_used', 'duration_ms', 'verified_at'],
    });
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${safeName}_verified.csv"`);
    res.send(csv);
  } catch (err) {
