...
const VERDICT_STYLES = {
  valid:   { dot: 'bg-emerald-500', text: 'text-emerald-700', bg: 'bg-emerald-50',  border: 'border-emerald-200', label: 'Valid' },
  invalid: { dot: 'bg-red-500',     text: 'text-red-700',     bg: 'bg-red-50',      border: 'border-red-200',     label: 'Invalid' },
  risky:   { dot: 'bg-amber-500',   text: 'text-amber-700',   bg: 'bg-amber-50',    border: 'border-amber-200',   label: 'Risky' },
  unknown: { dot: 'bg-zinc-400',    text: 'text-zinc-700',    bg: 'bg-zinc-50',     border: 'border-zinc-200',    label: 'Unknown' },
};
...
export default function SingleVerify({ onVerified }) {
  ...
  return (
    <section className="rounded-2xl border border-zinc-200 bg-white shadow-card p-6">
      ...
      {result && style && (
        <div className={`mt-4 rounded-lg border ${style.border} ${style.bg} p-3.5`}>
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${style.dot}`} />
            <span className={`text-[13px] font-semibold ${style.text}`}>{style.label}</span>
            <span className="ml-auto text-[11px] text-zinc-500 font-mono">{result.durationMs}ms</span>
          </div>
          ...
          {result.ipUsed && (
            <div className="mt-1.5 text-[10px] text-zinc-400 font-mono">via {result.ipUsed}</div>
          )}
        </div>
      )}
    </section>
